...
(function () {
  ...
  function applyPuzzleInstance(preservePuzzleState) {
    ...
    if (!preservePuzzleState) window.NaturalMergeSortExecutor.reset();
  ...
  function applyI18n() {
    ...
    if (window.NaturalMergeSortExecutor && window.NaturalMergeSortExecutor.refreshTrace) window.NaturalMergeSortExecutor.refreshTrace();
  }
  ...
  function bindSplitter() {
    ...
    const saved = Number(localStorage.getItem("nms-split"));
    ...
    function setSplitFromClientX(clientX) {
      ...
      localStorage.setItem("nms-split", String(Math.round(percent)));
    ...
    splitter.addEventListener("keydown", (e) => {
      ...
      e.preventDefault();
      const current = Number(localStorage.getItem("nms-split")) || 50;
      const next = e.key === "ArrowLeft" ? current - 3 : current + 3;
      const percent = Math.min(75, Math.max(25, next));
      left.style.flex = `1 1 ${percent}%`;
      right.style.flex = `1 1 ${100 - percent}%`;
      localStorage.setItem("nms-split", String(percent));
      window.ArrayViz.resize();
    });
  }
  ...
  function bindRowSplitter() {
    ...
    function applyHeight(px) {
      ...
      localStorage.setItem("nms-trace-h", String(clamped));
      if (window.ArrayViz && window.ArrayViz.resize) window.ArrayViz.resize();
    }

    const saved = Number(localStorage.getItem("nms-trace-h"));
    if (saved && saved >= minH) {
      applyHeight(saved);
    } else {
      applyHeight(220);
    ...
    splitter.addEventListener("keydown", (e) => {
      ...
      e.preventDefault();
      const current = Number(localStorage.getItem("nms-trace-h")) || 220;
      const next = e.key === "ArrowUp" ? current + 16 : current - 16;
      applyHeight(next);
    });

    window.addEventListener("resize", () => {
      const current = Number(localStorage.getItem("nms-trace-h")) || trace.getBoundingClientRect().height || 220;
      applyHeight(current);
    });
  }
  ...
  function bindControls() {
    ...
    document.getElementById("btn-run").addEventListener("click", () => {
      window.NaturalMergeSortExecutor.setSpeed(speedInput.value);
      window.NaturalMergeSortExecutor.run();
    });

    document.getElementById("btn-pause").addEventListener("click", () => {
      window.NaturalMergeSortExecutor.pause();
    });

    document.getElementById("btn-step").addEventListener("click", () => {
      window.NaturalMergeSortExecutor.setSpeed(speedInput.value);
      window.NaturalMergeSortExecutor.step();
    });

    document.getElementById("btn-rerun").addEventListener("click", () => {
      window.NaturalMergeSortExecutor.setSpeed(speedInput.value);
      window.ArrayViz.setArray(currentValues);
      window.NaturalMergeSortExecutor.rerun();
    });

    document.getElementById("btn-reset").addEventListener("click", () => {
      window.NaturalMergeSortExecutor.reset();
      window.ArrayViz.setArray(currentValues);
    });
  ...
  window.addEventListener("load", () => {
    ...
    window.NaturalMergeSortExecutor.init();
