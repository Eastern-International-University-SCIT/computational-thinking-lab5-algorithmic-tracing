...
(function () {
  ...
  function applyPuzzleInstance(preservePuzzleState) {
    ...
    if (!preservePuzzleState) window.MergeSortExecutor.reset();
  ...
  function applyI18n() {
    ...
    if (window.MergeSortExecutor && window.MergeSortExecutor.refreshTrace) window.MergeSortExecutor.refreshTrace();
  }
  ...
  function bindSplitter() {
    ...
    const saved = Number(localStorage.getItem("ms-split"));
    ...
    function setSplitFromClientX(clientX) {
      ...
      localStorage.setItem("ms-split", String(Math.round(percent)));
    ...
    splitter.addEventListener("keydown", (e) => {
      ...
      e.preventDefault();
      const current = Number(localStorage.getItem("ms-split")) || 50;
      const next = e.key === "ArrowLeft" ? current - 3 : current + 3;
      const percent = Math.min(75, Math.max(25, next));
      left.style.flex = `1 1 ${percent}%`;
      right.style.flex = `1 1 ${100 - percent}%`;
      localStorage.setItem("ms-split", String(percent));
      window.ArrayViz.resize();
    });
  }
  ...
  function bindRowSplitter() {
    ...
    function applyHeight(px) {
      ...
      localStorage.setItem("ms-trace-h", String(clamped));
      if (window.ArrayViz && window.ArrayViz.resize) window.ArrayViz.resize();
    }

    const saved = Number(localStorage.getItem("ms-trace-h"));
    if (saved && saved >= minH) {
      applyHeight(saved);
    } else {
      applyHeight(220);
    ...
    splitter.addEventListener("keydown", (e) => {
      ...
      e.preventDefault();
      const current = Number(localStorage.getItem("ms-trace-h")) || 220;
      const next = e.key === "ArrowUp" ? current + 16 : current - 16;
      applyHeight(next);
    });

    window.addEventListener("resize", () => {
      const current = Number(localStorage.getItem("ms-trace-h")) || trace.getBoundingClientRect().height || 220;
      applyHeight(current);
    });
  }
  ...
  function bindControls() {
    ...
    document.getElementById("btn-run").addEventListener("click", () => {
      window.MergeSortExecutor.setSpeed(speedInput.value);
      window.MergeSortExecutor.run();
    });

    document.getElementById("btn-pause").addEventListener("click", () => {
      window.MergeSortExecutor.pause();
    });

    document.getElementById("btn-step").addEventListener("click", () => {
      window.MergeSortExecutor.setSpeed(speedInput.value);
      window.MergeSortExecutor.step();
    });

    document.getElementById("btn-rerun").addEventListener("click", () => {
      window.MergeSortExecutor.setSpeed(speedInput.value);
      window.ArrayViz.setArray(currentValues);
      window.MergeSortExecutor.rerun();
    });

    document.getElementById("btn-reset").addEventListener("click", () => {
      window.MergeSortExecutor.reset();
      window.ArrayViz.setArray(currentValues);
    });
