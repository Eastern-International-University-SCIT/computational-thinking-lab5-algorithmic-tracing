...
(function () {
  ...
  function applyPuzzleInstance(preservePuzzleState) {
    ...
    if (!preservePuzzleState) window.QuickSortExecutor.reset();
  ...
  function applyI18n() {
    ...
    if (window.QuickSortExecutor && window.QuickSortExecutor.refreshTrace) window.QuickSortExecutor.refreshTrace();
  }
  ...
  function bindSplitter() {
    ...
    const saved = Number(localStorage.getItem("qs-split"));
    ...
    function setSplitFromClientX(clientX) {
      ...
      localStorage.setItem("qs-split", String(Math.round(percent)));
    ...
    splitter.addEventListener("keydown", (e) => {
      ...
      e.preventDefault();
      const current = Number(localStorage.getItem("qs-split")) || 50;
      const next = e.key === "ArrowLeft" ? current - 3 : current + 3;
      const percent = Math.min(75, Math.max(25, next));
      left.style.flex = `1 1 ${percent}%`;
      right.style.flex = `1 1 ${100 - percent}%`;
      localStorage.setItem("qs-split", String(percent));
      window.ArrayViz.resize();
    });
  }
  ...
  function bindRowSplitter() {
    ...
    function applyHeight(px) {
      ...
      localStorage.setItem("qs-trace-h", String(clamped));
      if (window.ArrayViz && window.ArrayViz.resize) window.ArrayViz.resize();
    }

    const saved = Number(localStorage.getItem("qs-trace-h"));
    if (saved && saved >= minH) {
      applyHeight(saved);
    } else {
      applyHeight(220);
    ...
    splitter.addEventListener("keydown", (e) => {
      ...
      e.preventDefault();
      const current = Number(localStorage.getItem("qs-trace-h")) || 220;
      const next = e.key === "ArrowUp" ? current + 16 : current - 16;
      applyHeight(next);
    });

    window.addEventListener("resize", () => {
      const current = Number(localStorage.getItem("qs-trace-h")) || trace.getBoundingClientRect().height || 220;
      applyHeight(current);
    });
  }
  ...
  function bindControls() {
    ...
    document.getElementById("btn-run").addEventListener("click", () => {
      window.QuickSortExecutor.setSpeed(speedInput.value);
      window.QuickSortExecutor.run();
    });

    document.getElementById("btn-pause").addEventListener("click", () => {
      window.QuickSortExecutor.pause();
    });

    document.getElementById("btn-step").addEventListener("click", () => {
      window.QuickSortExecutor.setSpeed(speedInput.value);
      window.QuickSortExecutor.step();
    });

    document.getElementById("btn-rerun").addEventListener("click", () => {
      window.QuickSortExecutor.setSpeed(speedInput.value);
      window.ArrayViz.setArray(currentValues);
      window.QuickSortExecutor.rerun();
    });

    document.getElementById("btn-reset").addEventListener("click", () => {
      window.QuickSortExecutor.reset();
      window.ArrayViz.setArray(currentValues);
    });
