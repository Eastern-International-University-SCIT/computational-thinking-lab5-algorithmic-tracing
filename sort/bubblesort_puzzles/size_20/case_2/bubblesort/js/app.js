...
(function () {
  ...
  function applyI18n() {
    ...
    if (window.BubbleSortExecutor && window.BubbleSortExecutor.refreshTrace) window.BubbleSortExecutor.refreshTrace();
  }
  ...
  function bindRowSplitter() {
    ...
    splitter.addEventListener("keydown", (e) => {
      ...
      e.preventDefault();
      const current = Number(localStorage.getItem("bs-trace-h")) || 220;
      const next = e.key === "ArrowUp" ? current + 16 : current - 16;
      applyHeight(next);
    });

    window.addEventListener("resize", () => {
      const current = Number(localStorage.getItem("bs-trace-h")) || trace.getBoundingClientRect().height || 220;
      applyHeight(current);
    });
  }
  ...
  function bindControls() {
    ...
    document.getElementById("btn-run").addEventListener("click", () => {
      window.BubbleSortExecutor.setSpeed(speedInput.value);
      window.BubbleSortExecutor.run();
    });

    document.getElementById("btn-pause").addEventListener("click", () => {
      window.BubbleSortExecutor.pause();
    });

    document.getElementById("btn-step").addEventListener("click", () => {
      window.BubbleSortExecutor.setSpeed(speedInput.value);
      window.BubbleSortExecutor.step();
    });

    document.getElementById("btn-rerun").addEventListener("click", () => {
      window.BubbleSortExecutor.setSpeed(speedInput.value);
      window.ArrayViz.setArray(currentValues);
      window.BubbleSortExecutor.rerun();
    });
  ...
  window.addEventListener("load", () => {
    ...
    window.BubbleSortExecutor.init();
