/**
 * p5.js WEBGL visualization for Selection Sort.
 * Array as square 3D boxes; amber = current minIndex; i / j pointers below.
 */
...
window.ArrayViz = (function () {
  ...
  /**
   * Place cubes with equal side gaps, then scale the row to fit the canvas.
   */
  ...
  function colorFor(i) {
    ...
    if (i === state.minIndex) return COLORS.min;
  ...
  function drawMinMarker(p, x, topY, size) {
    const s = state.fitScale;
    p.push();
    useSolidFill(p, COLORS.min);
    ...
    p.textureMode(p.NORMAL);
    p.texture(
      makeLabel(p, labelText("labelMin", "min"), {
        w: 100,
        h: 36,
        size: 16,
        bg: [217, 119, 6, 235],
  ...
  function drawIndexPointer(
    ...
    p.plane(Math.max(18, size * 0.42), labelPlaneH);
    p.pop();
  ...
  const sketch = function (p) {
    ...
    p.draw = function () {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (i === state.minIndex) {
          drawMinMarker(p, x, baseY - size + yLift, size);
        }
      }

      // i / j pointers use layout slots so they stay put while boxes swap
  ...
  function markSorted(index) {
    ...
    if (state.minIndex === index) state.minIndex = -1;
  }
