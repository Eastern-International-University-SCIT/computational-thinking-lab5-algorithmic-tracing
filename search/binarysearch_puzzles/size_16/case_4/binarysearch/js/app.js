...
(function () {
  ...
  function applyPuzzleInstance(preservePuzzleState) {
    ...
    if (!preservePuzzleState) window.BinarySearchExecutor.reset();
    ...
    window.BinarySearchExecutor.setKey(inst.key);
  }
  ...
  function applyI18n() {
    ...
    if (window.BinarySearchExecutor && window.BinarySearchExecutor.refreshTrace) {
      window.BinarySearchExecutor.refreshTrace();
    }
  }
  ...
  function bindSplitter() {
    ...
    const saved = Number(localStorage.getItem("bs-split"));
    ...
    function setSplitFromClientX(clientX) {
      ...
      localStorage.setItem("bs-split", String(Math.round(percent)));
    ...
    splitter.addEventListener("keydown", (e) => {
      ...
      e.preventDefault();
      const current = Number(localStorage.getItem("bs-split")) || 50;
      const next = e.key === "ArrowLeft" ? current - 3 : current + 3;
      const percent = Math.min(75, Math.max(25, next));
      left.style.flex = `1 1 ${percent}%`;
      right.style.flex = `1 1 ${100 - percent}%`;
      localStorage.setItem("bs-split", String(percent));
      window.ArrayViz.resize();
    });
  }
  ...
  function bindRowSplitter() {
    ...
    function applyHeight(px) {
      ...
      localStorage.setItem("bs-trace-h", String(clamped));
      if (window.ArrayViz && window.ArrayViz.resize) window.ArrayViz.resize();
    }

    const saved = Number(localStorage.getItem("bs-trace-h"));
    if (saved && saved >= minH) {
      applyHeight(saved);
    } else {
      applyHeight(220);
    ...
    splitter.addEventListener("keydown", (e) => {
      ...
      e.preventDefault();
      const current =
        Number(localStorage.getItem("bs-trace-h")) || 220;
    ...
    window.addEventListener("resize", () => {
      const current =
        Number(localStorage.getItem("bs-trace-h")) ||
  ...
  function bindControls() {
    ...
    document.getElementById("btn-run").addEventListener("click", () => {
      window.BinarySearchExecutor.setSpeed(speedInput.value);
      window.BinarySearchExecutor.setKey(keyInput.value);
      window.BinarySearchExecutor.run();
    });

    document.getElementById("btn-pause").addEventListener("click", () => {
      window.BinarySearchExecutor.pause();
    });

    document.getElementById("btn-step").addEventListener("click", () => {
      window.BinarySearchExecutor.setSpeed(speedInput.value);
      window.BinarySearchExecutor.setKey(keyInput.value);
      window.BinarySearchExecutor.step();
    });
    ...
    document.getElementById("btn-rerun").addEventListener("click", () => {
      ...
      window.ArrayViz.setArray(currentValues);
      window.BinarySearchExecutor.setKey(keyInput.value);
      window.BinarySearchExecutor.rerun();
    });

    document.getElementById("btn-reset").addEventListener("click", () => {
      window.BinarySearchExecutor.reset();
      window.ArrayViz.setArray(currentValues);
      window.BinarySearchExecutor.setKey(keyInput.value);
    });
  ...
  window.addEventListener("load", () => {
    ...
    window.BinarySearchExecutor.init();
