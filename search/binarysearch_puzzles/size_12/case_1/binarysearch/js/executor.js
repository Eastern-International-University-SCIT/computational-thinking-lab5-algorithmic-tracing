...
window.BinarySearchExecutor = (function () {
  ...
  function syncPointers() {
    if (window.ArrayViz && window.ArrayViz.setPointers) {
      window.ArrayViz.setPointers(vars.low, vars.mid, vars.high);
    }
  }
  ...
  async function binarySearch(n, key) {
    const step = ids();
    vars.n = n;
    vars.key = key;
    ...
    await doStep(step.returnMiss, async () => {
      ...
      window.ArrayViz.clearHighlights();
      window.ArrayViz.markRange(-1, -1);
      // Highlight the last mid (or center) as miss cue when possible
      let missAt = vars.mid;
  ...
  async function runProgram() {
    ...
    try {
      ...
      await binarySearch(n, searchKey);
      clearHighlight();
    } catch (err) {
