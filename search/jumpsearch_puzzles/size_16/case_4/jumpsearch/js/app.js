...
(function () {
  ...
  function applyPuzzleInstance(preservePuzzleState) {
    ...
    if (!preservePuzzleState) window.JumpSearchExecutor.reset();
    ...
    window.JumpSearchExecutor.setKey(inst.key);
  }
  ...
  function applyI18n() {
    ...
    if (window.JumpSearchExecutor && window.JumpSearchExecutor.refreshTrace) {
      window.JumpSearchExecutor.refreshTrace();
    }
  }
  ...
  function bindSplitter() {
    ...
    const saved = Number(localStorage.getItem("js-split"));
    ...
    function setSplitFromClientX(clientX) {
      ...
      localStorage.setItem("js-split", String(Math.round(percent)));
    ...
    splitter.addEventListener("keydown", (e) => {
      ...
      e.preventDefault();
      const current = Number(localStorage.getItem("js-split")) || 50;
      const next = e.key === "ArrowLeft" ? current - 3 : current + 3;
      const percent = Math.min(75, Math.max(25, next));
      left.style.flex = `1 1 ${percent}%`;
      right.style.flex = `1 1 ${100 - percent}%`;
      localStorage.setItem("js-split", String(percent));
      window.ArrayViz.resize();
    });
  }
  ...
  function bindRowSplitter() {
    ...
    function applyHeight(px) {
      ...
      localStorage.setItem("js-trace-h", String(clamped));
      if (window.ArrayViz && window.ArrayViz.resize) window.ArrayViz.resize();
    }

    const saved = Number(localStorage.getItem("js-trace-h"));
    if (saved && saved >= minH) {
      applyHeight(saved);
    } else {
      applyHeight(220);
    ...
    splitter.addEventListener("keydown", (e) => {
      ...
      e.preventDefault();
      const current =
        Number(localStorage.getItem("js-trace-h")) || 220;
    ...
    window.addEventListener("resize", () => {
      const current =
        Number(localStorage.getItem("js-trace-h")) ||
  ...
  function bindControls() {
    ...
    document.getElementById("btn-run").addEventListener("click", () => {
      window.JumpSearchExecutor.setSpeed(speedInput.value);
      window.JumpSearchExecutor.setKey(keyInput.value);
      window.JumpSearchExecutor.run();
    });

    document.getElementById("btn-pause").addEventListener("click", () => {
      window.JumpSearchExecutor.pause();
    });

    document.getElementById("btn-step").addEventListener("click", () => {
      window.JumpSearchExecutor.setSpeed(speedInput.value);
      window.JumpSearchExecutor.setKey(keyInput.value);
      window.JumpSearchExecutor.step();
    });
    ...
    document.getElementById("btn-rerun").addEventListener("click", () => {
      ...
      window.ArrayViz.setArray(currentValues);
      window.JumpSearchExecutor.setKey(keyInput.value);
      window.JumpSearchExecutor.rerun();
    });

    document.getElementById("btn-reset").addEventListener("click", () => {
      window.JumpSearchExecutor.reset();
      window.ArrayViz.setArray(currentValues);
      window.JumpSearchExecutor.setKey(keyInput.value);
    });
  ...
  window.addEventListener("load", () => {
    ...
    window.JumpSearchExecutor.init();
