...
(function () {
  ...
  function applyPuzzleInstance(preservePuzzleState) {
    ...
    window.InterpolationSearchExecutor.setKey(inst.key);
  }
  ...
  function applyI18n() {
    ...
    if (window.InterpolationSearchExecutor && window.InterpolationSearchExecutor.refreshTrace) {
      window.InterpolationSearchExecutor.refreshTrace();
    }
  }
  ...
  function bindSplitter() {
    ...
    const saved = Number(localStorage.getItem("is-split"));
    ...
    function setSplitFromClientX(clientX) {
      ...
      localStorage.setItem("is-split", String(Math.round(percent)));
    ...
    splitter.addEventListener("keydown", (e) => {
      ...
      e.preventDefault();
      const current = Number(localStorage.getItem("is-split")) || 50;
      const next = e.key === "ArrowLeft" ? current - 3 : current + 3;
      const percent = Math.min(75, Math.max(25, next));
      left.style.flex = `1 1 ${percent}%`;
      right.style.flex = `1 1 ${100 - percent}%`;
      localStorage.setItem("is-split", String(percent));
      window.ArrayViz.resize();
    });
  }
  ...
  function bindRowSplitter() {
    ...
    function applyHeight(px) {
      ...
      localStorage.setItem("is-trace-h", String(clamped));
      if (window.ArrayViz && window.ArrayViz.resize) window.ArrayViz.resize();
    }

    const saved = Number(localStorage.getItem("is-trace-h"));
    if (saved && saved >= minH) {
      applyHeight(saved);
    } else {
      applyHeight(220);
    ...
    splitter.addEventListener("keydown", (e) => {
      ...
      e.preventDefault();
      const current =
        Number(localStorage.getItem("is-trace-h")) || 220;
    ...
    window.addEventListener("resize", () => {
      const current =
        Number(localStorage.getItem("is-trace-h")) ||
  ...
  function bindControls() {
    ...
    document.getElementById("btn-run").addEventListener("click", () => {
      window.InterpolationSearchExecutor.setSpeed(speedInput.value);
      window.InterpolationSearchExecutor.setKey(keyInput.value);
      window.InterpolationSearchExecutor.run();
    });

    document.getElementById("btn-pause").addEventListener("click", () => {
      window.InterpolationSearchExecutor.pause();
    });

    document.getElementById("btn-step").addEventListener("click", () => {
      window.InterpolationSearchExecutor.setSpeed(speedInput.value);
      window.InterpolationSearchExecutor.setKey(keyInput.value);
      window.InterpolationSearchExecutor.step();
    });
    ...
    document.getElementById("btn-rerun").addEventListener("click", () => {
      ...
      window.ArrayViz.setArray(currentValues);
      window.InterpolationSearchExecutor.setKey(keyInput.value);
      window.InterpolationSearchExecutor.rerun();
    });
  ...
  window.addEventListener("load", () => {
    ...
    window.InterpolationSearchExecutor.init();
