/**
 * p5.js WEBGL visualization for Interpolation Search.
 * Sorted array as square 3D boxes; low / pos / high pointers; search-range highlight.
 */
...
window.ArrayViz = (function () {
  ...
  const state = {
    ...
    missIndex: -1,
    range: { low: -1, high: -1 },
    pointers: { low: null, pos: null, high: null },
    compare: null,
    compareAnim: null,
    keyValue: null,
    keyDisplayX: 0,
    ...
    p.draw = function () {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (i === state.foundIndex) {
          ...
          state.pointers.pos != null &&
          state.pointers.pos === i &&
          state.foundIndex < 0
        ) {
          drawTopMarker(
            p,
      ...
      const showPosBottom =
        isValidPointer(state.pointers.pos) &&
        (state.pointers.pos < 0 || state.pointers.pos >= n);
      ...
      if (showPosBottom) {
        bottomPtrs.push({
          idx: state.pointers.pos,
          text: labelText("labelPos", "pos"),
          color: COLORS.pointerPos,
        });
      }
  ...
  function setArray(values) {
    ...
    state.missIndex = -1;
    state.highlight = -1;
    state.pointers = { low: null, pos: null, high: null };
  ...
  function setPointers(low, pos, high) {
    state.pointers = {
      low: low == null || !Number.isFinite(low) ? null : low,
      pos: pos == null || !Number.isFinite(pos) ? null : pos,
