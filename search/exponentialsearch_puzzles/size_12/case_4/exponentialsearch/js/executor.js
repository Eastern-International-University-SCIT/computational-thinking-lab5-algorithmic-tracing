...
window.ExponentialSearchExecutor = (function () {
  let running = false;
  let paused = false;
  let abort = false;
  let stepMode = false;
  ...
  function ids() {
    return window.ES_STEP_IDS || window.PseudocodeView.STEP_IDS;
  }
  ...
  function markExpRange() {
    if (!window.ArrayViz || vars.bound == null || vars.n == null) return;
    const high = Math.min(vars.bound, vars.n - 1);
    if (high >= 0) {
      window.ArrayViz.markRange(0, high);
    } else {
  ...
  async function exponentialSearch(n, key) {
    const step = ids();
    vars.n = n;
    vars.key = key;
    ...
    await doStep(step.checkFirst, async () => {
      if (n > 0) {
        firstEqual = await animateCompare(0, key, "=");
      } else {
        firstEqual = false;
      }
    });
    ...
    if (firstEqual) {
      await doStep(step.returnZero, async () => {
        vars.result = 0;
        window.ArrayViz.clearHighlights();
        window.ArrayViz.markRange(-1, -1);
        window.ArrayViz.setFoundIndex(0);
        showFound(key, 0);
      });
      return;
    }

    await doStep(step.setBound, async () => {
      vars.bound = 1;
      markExpRange();
    });
    ...
    while (true) {
      ...
      let expCond = false;
      await doStep(step.whileExp, async () => {
        markExpRange();
        const boundLtN = vars.bound < n;
        if (!boundLtN) {
          expCond = false;
          const text = window.I18n
            ? window.I18n.compareText(vars.bound, n, "<", false)
            : `Compare: bound < n (${vars.bound} < ${n}) → false`;
          window.ArrayViz.setCompare(-1, -1, text);
        } else {
          expCond = await animateCompare(vars.bound, key, "<");
        }
      });
      window.ArrayViz.clearCompare();
      ...
      await doStep(step.doubleBound, async () => {
        vars.bound = vars.bound * 2;
        markExpRange();
      });
    }

    // Binary search phase in [⌊bound/2⌋ .. min(bound, n−1)]
    await doStep(step.setLow, async () => {
      vars.low = Math.floor(vars.bound / 2);
      markSearchRange();
    });

    await doStep(step.setHigh, async () => {
      vars.high = Math.min(vars.bound, n - 1);
      markSearchRange();
    });
    ...
    while (true) {
      ...
      await doStep(step.whileLoop, async () => {
        loopCond = vars.low <= vars.high;
        const text = window.I18n
          ? window.I18n.compareText(vars.low, vars.high, "≤", loopCond)
          : `Compare: low ≤ high (${vars.low} ≤ ${vars.high}) → ${loopCond}`;
        window.ArrayViz.setCompare(-1, -1, text);
        markSearchRange();
      });
      window.ArrayViz.clearCompare();
      ...
      await doStep(step.setMid, async () => {
        vars.mid = Math.floor((vars.low + vars.high) / 2);
        markSearchRange();
        if (vars.mid >= 0 && vars.mid < n) {
          window.ArrayViz.setHighlight(vars.mid);
        }
      });
      ...
      await doStep(step.checkEqual, async () => {
        if (vars.mid >= 0 && vars.mid < n) {
          equal = await animateCompare(vars.mid, key, "=");
        } else {
          equal = false;
        }
      });
      ...
      if (equal) {
        await doStep(step.returnFound, async () => {
          vars.result = vars.mid;
          window.ArrayViz.clearHighlights();
          window.ArrayViz.markRange(-1, -1);
          window.ArrayViz.setFoundIndex(vars.mid);
          showFound(key, vars.mid);
        });
        return;
      }
      ...
      await doStep(step.checkLess, async () => {
        if (vars.mid >= 0 && vars.mid < n) {
          less = await animateCompare(vars.mid, key, "<");
        } else {
          less = false;
        }
      });
    ...
    await doStep(step.returnMiss, async () => {
      ...
      if (missAt == null || missAt < 0 || missAt >= n) {
        const lo = vars.low != null ? vars.low : 0;
        const hi = vars.high != null ? vars.high : n - 1;
        missAt = n > 0 ? Math.min(n - 1, Math.max(0, Math.floor((lo + hi) / 2))) : -1;
      }
  ...
  async function runProgram() {
    ...
    try {
      ...
      await exponentialSearch(n, searchKey);
      clearHighlight();
    } catch (err) {
