/**
 * p5.js WEBGL visualization for Exponential Search.
 * Sorted array as square 3D boxes; bound then low / mid / high pointers.
 */
...
window.ArrayViz = (function () {
  const COLORS = {
    ...
    purple: [128, 90, 213],
    keyBox: [46, 160, 144],
    pointerBound: [217, 119, 6],
    pointerBoundGrey: [148, 156, 164],
    pointerLow: [61, 90, 128],
    pointerMid: [91, 106, 191],
    pointerHigh: [196, 92, 74],
  };
  ...
  const state = {
    ...
    missIndex: -1,
    range: { low: -1, high: -1 },
    pointers: { low: null, mid: null, high: null, bound: null },
    compare: null,
    compareAnim: null,
    keyValue: null,
    keyDisplayX: 0,
  ...
  /**
   * Upward arrow + label under a box.
   */
    ...
    p.draw = function () {
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (i === state.foundIndex) {
          ...
          state.pointers.bound != null &&
          state.pointers.bound === i &&
          state.foundIndex < 0 &&
          state.missIndex < 0 &&
          (state.pointers.mid == null || state.pointers.mid !== i)
        ) {
          drawTopMarker(
            p,
            state.targetPositions[i],
      ...
      const showLow = isValidPointer(state.pointers.low);
      const showHigh = isValidPointer(state.pointers.high);
      const showMidBottom =
        isValidPointer(state.pointers.mid) &&
        (state.pointers.mid < 0 || state.pointers.mid >= n);
      // Keep bound visible after the exponential while (binary phase too)
      const showBoundBottom = isValidPointer(state.pointers.bound);
      const boundOutOfRange =
        showBoundBottom &&
        (state.pointers.bound < 0 || state.pointers.bound >= n);

      // Collect bottom pointers and nudge when they share a slot
      const bottomPtrs = [];
      if (showLow) {
        bottomPtrs.push({
          idx: state.pointers.low,
          text: labelText("labelLow", "low"),
          color: COLORS.pointerLow,
        });
      }
      if (showHigh) {
        bottomPtrs.push({
          idx: state.pointers.high,
          text: labelText("labelHigh", "high"),
          color: COLORS.pointerHigh,
        });
      }
      if (showMidBottom) {
        bottomPtrs.push({
          idx: state.pointers.mid,
          text: labelText("labelMid", "mid"),
          color: COLORS.pointerMid,
        });
      }
      if (showBoundBottom) {
        bottomPtrs.push({
          idx: state.pointers.bound,
          text: labelText("labelBound", "bound"),
          color: boundOutOfRange
            ? COLORS.pointerBoundGrey
            : COLORS.pointerBound,
        });
      }

      const bySlot = new Map();
      bottomPtrs.forEach((ptr) => {
        const key = String(ptr.idx);
        if (!bySlot.has(key)) bySlot.set(key, []);
        bySlot.get(key).push(ptr);
      });
      ...
      bySlot.forEach((group) => {
        ...
        group.forEach((ptr, i) => {
          const size =
            ptr.idx >= 0 && ptr.idx < n
              ? state.sizes[ptr.idx] || 40
              : 40 * state.fitScale;
          const x = xAtIndex(ptr.idx);
          let shift = 0;
          if (count === 2) shift = (i === 0 ? -1 : 1) * 14 * state.fitScale;
          if (count >= 3) shift = (i - 1) * 16 * state.fitScale;
          drawIndexPointer(p, ptr.text, x, baseY, size, ptr.color, shift);
        });
      });
    };
  ...
  function setArray(values) {
    ...
    state.missIndex = -1;
    state.highlight = -1;
    state.pointers = { low: null, mid: null, high: null, bound: null };
  ...
  function setPointers(low, mid, high, bound) {
    state.pointers = {
      low: low == null || !Number.isFinite(low) ? null : low,
      mid: mid == null || !Number.isFinite(mid) ? null : mid,
      high: high == null || !Number.isFinite(high) ? null : high,
      bound: bound == null || !Number.isFinite(bound) ? null : bound,
    };
  }
  ...
  function setResultMessage(type, key, position) {
    state.resultMessage = type ? { type, key, position } : null;
    updateResultHud();
  }
