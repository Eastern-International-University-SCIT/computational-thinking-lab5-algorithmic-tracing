/**
 * App bootstrap: puzzle mode locks the array/key to the fixed instance.
 */
...
(function () {
  ...
  function applyPuzzleInstance(preservePuzzleState) {
    ...
    if (!preservePuzzleState) window.ExponentialSearchExecutor.reset();
    ...
    window.ExponentialSearchExecutor.setKey(inst.key);
  }
  ...
  function applyI18n() {
    ...
    if (window.ExponentialSearchExecutor && window.ExponentialSearchExecutor.refreshTrace) {
      window.ExponentialSearchExecutor.refreshTrace();
    }
  }
  ...
  function bindSplitter() {
    ...
    const saved = Number(localStorage.getItem("es-split"));
    ...
    function setSplitFromClientX(clientX) {
      ...
      localStorage.setItem("es-split", String(Math.round(percent)));
    ...
    splitter.addEventListener("keydown", (e) => {
      ...
      e.preventDefault();
      const current = Number(localStorage.getItem("es-split")) || 50;
      const next = e.key === "ArrowLeft" ? current - 3 : current + 3;
      const percent = Math.min(75, Math.max(25, next));
      left.style.flex = `1 1 ${percent}%`;
      right.style.flex = `1 1 ${100 - percent}%`;
      localStorage.setItem("es-split", String(percent));
      window.ArrayViz.resize();
    });
  }
  ...
  function bindRowSplitter() {
    ...
    function applyHeight(px) {
      ...
      localStorage.setItem("es-trace-h", String(clamped));
      if (window.ArrayViz && window.ArrayViz.resize) window.ArrayViz.resize();
    }

    const saved = Number(localStorage.getItem("es-trace-h"));
    if (saved && saved >= minH) {
      applyHeight(saved);
    } else {
      applyHeight(220);
    ...
    splitter.addEventListener("keydown", (e) => {
      ...
      e.preventDefault();
      const current =
        Number(localStorage.getItem("es-trace-h")) || 220;
    ...
    window.addEventListener("resize", () => {
      const current =
        Number(localStorage.getItem("es-trace-h")) ||
  ...
  function bindControls() {
    ...
    document.getElementById("btn-run").addEventListener("click", () => {
      window.ExponentialSearchExecutor.setSpeed(speedInput.value);
      window.ExponentialSearchExecutor.setKey(keyInput.value);
      window.ExponentialSearchExecutor.run();
    });

    document.getElementById("btn-pause").addEventListener("click", () => {
      window.ExponentialSearchExecutor.pause();
    });

    document.getElementById("btn-step").addEventListener("click", () => {
      window.ExponentialSearchExecutor.setSpeed(speedInput.value);
      window.ExponentialSearchExecutor.setKey(keyInput.value);
      window.ExponentialSearchExecutor.step();
    });
    ...
    document.getElementById("btn-rerun").addEventListener("click", () => {
      ...
      window.ArrayViz.setArray(currentValues);
      window.ExponentialSearchExecutor.setKey(keyInput.value);
      window.ExponentialSearchExecutor.rerun();
    });

    document.getElementById("btn-reset").addEventListener("click", () => {
      window.ExponentialSearchExecutor.reset();
      window.ArrayViz.setArray(currentValues);
      window.ExponentialSearchExecutor.setKey(keyInput.value);
    });
  ...
  window.addEventListener("load", () => {
    ...
    window.ExponentialSearchExecutor.init();
