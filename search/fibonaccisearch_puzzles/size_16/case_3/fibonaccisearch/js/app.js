...
(function () {
  ...
  function applyPuzzleInstance(preservePuzzleState) {
    ...
    if (!preservePuzzleState) window.FibonacciSearchExecutor.reset();
    ...
    window.FibonacciSearchExecutor.setKey(inst.key);
  }
  ...
  function applyI18n() {
    ...
    if (window.FibonacciSearchExecutor && window.FibonacciSearchExecutor.refreshTrace) {
      window.FibonacciSearchExecutor.refreshTrace();
    }
  }
  ...
  function bindSplitter() {
    ...
    const saved = Number(localStorage.getItem("fs-split"));
    ...
    function setSplitFromClientX(clientX) {
      ...
      localStorage.setItem("fs-split", String(Math.round(percent)));
    ...
    splitter.addEventListener("keydown", (e) => {
      ...
      e.preventDefault();
      const current = Number(localStorage.getItem("fs-split")) || 50;
      const next = e.key === "ArrowLeft" ? current - 3 : current + 3;
      const percent = Math.min(75, Math.max(25, next));
      left.style.flex = `1 1 ${percent}%`;
      right.style.flex = `1 1 ${100 - percent}%`;
      localStorage.setItem("fs-split", String(percent));
      window.ArrayViz.resize();
    });
  }
  ...
  function bindRowSplitter() {
    ...
    function applyHeight(px) {
      ...
      localStorage.setItem("fs-trace-h", String(clamped));
      if (window.ArrayViz && window.ArrayViz.resize) window.ArrayViz.resize();
    }

    const saved = Number(localStorage.getItem("fs-trace-h"));
    if (saved && saved >= minH) {
      applyHeight(saved);
    } else {
      applyHeight(220);
    ...
    splitter.addEventListener("keydown", (e) => {
      ...
      e.preventDefault();
      const current =
        Number(localStorage.getItem("fs-trace-h")) || 220;
    ...
    window.addEventListener("resize", () => {
      const current =
        Number(localStorage.getItem("fs-trace-h")) ||
  ...
  function bindControls() {
    ...
    document.getElementById("btn-run").addEventListener("click", () => {
      window.FibonacciSearchExecutor.setSpeed(speedInput.value);
      window.FibonacciSearchExecutor.setKey(keyInput.value);
      window.FibonacciSearchExecutor.run();
    });

    document.getElementById("btn-pause").addEventListener("click", () => {
      window.FibonacciSearchExecutor.pause();
    });

    document.getElementById("btn-step").addEventListener("click", () => {
      window.FibonacciSearchExecutor.setSpeed(speedInput.value);
      window.FibonacciSearchExecutor.setKey(keyInput.value);
      window.FibonacciSearchExecutor.step();
    });
    ...
    document.getElementById("btn-rerun").addEventListener("click", () => {
      ...
      window.ArrayViz.setArray(currentValues);
      window.FibonacciSearchExecutor.setKey(keyInput.value);
      window.FibonacciSearchExecutor.rerun();
    });

    document.getElementById("btn-reset").addEventListener("click", () => {
      window.FibonacciSearchExecutor.reset();
      window.ArrayViz.setArray(currentValues);
      window.FibonacciSearchExecutor.setKey(keyInput.value);
    });
  ...
  window.addEventListener("load", () => {
    ...
    window.FibonacciSearchExecutor.init();
