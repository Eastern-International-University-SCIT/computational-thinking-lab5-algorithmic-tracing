/**
 * App bootstrap: puzzle mode locks the insert sequence to the fixed instance.
 */
...
(function () {
  ...
  function applyPuzzleInstance(preservePuzzleState) {
    ...
    if (!inst) return;
    const seq = (inst.sequence || inst.array || []).slice();
    ...
    if (!preservePuzzleState) window.BstInsertExecutor.reset();
  ...
  function applyI18n() {
    ...
    if (window.TracePuzzle && window.TracePuzzle.refreshLanguage) {
  ...
  function bindSplitter() {
    ...
    const saved = Number(localStorage.getItem("bsti-puzzle-split"));
    ...
    function setSplitFromClientX(clientX) {
      ...
      localStorage.setItem("bsti-puzzle-split", String(Math.round(percent)));
  ...
  function bindRowSplitter() {
    ...
    function applyHeight(px) {
      ...
      localStorage.setItem("bsti-puzzle-trace-h", String(clamped));
      if (window.TreeViz && window.TreeViz.resize) window.TreeViz.resize();
    }

    const saved = Number(localStorage.getItem("bsti-puzzle-trace-h"));
  ...
  function bindControls() {
    ...
    if (applyBtn) {
      ...
      applyBtn.title = "Locked to puzzle instance";
    }

    document.getElementById("btn-run").addEventListener("click", () => {
      window.BstInsertExecutor.setSpeed(speedInput.value);
      window.BstInsertExecutor.run();
    });

    document.getElementById("btn-pause").addEventListener("click", () => {
      window.BstInsertExecutor.pause();
    });

    document.getElementById("btn-step").addEventListener("click", () => {
      window.BstInsertExecutor.setSpeed(speedInput.value);
      window.BstInsertExecutor.step();
    });

    document.getElementById("btn-rerun").addEventListener("click", () => {
      window.BstInsertExecutor.setSpeed(speedInput.value);
      window.BstInsertExecutor.rerun();
    });
  ...
  window.addEventListener("load", () => {
    ...
    window.addEventListener("resize", () => window.TreeViz.resize());
  });
})();
